...
export function NewClientForm({ slug, orgId }: { slug: string; orgId: string }) {
  ...
  return (
    <form action={createClientAction} className="min-h-screen pb-28">
      ...
      <div className="mb-6">
        <nav className="flex items-center gap-1.5 text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="font-medium text-gray-700">New Client</span>
        </nav>
        <h1 className="mt-3 text-2xl font-bold text-gray-900">New Client</h1>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Client Name" required>
                <input name="name" required className={inputCls} placeholder="Acme Corporation" />
              </Field>
              <Field label="Client Type">
                <div className="relative">
                  <select name="clientType" defaultValue="company" className={`${inputCls} appearance-none pr-10`}>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Full Name">
                <input name="contactName" className={inputCls} placeholder="Sarah Johnson" />
              </Field>
              <Field label="Email Address">
                <input name="contactEmail" type="email" className={inputCls} placeholder="sarah@acme.com" />
              </Field>
              <Field label="Phone Number">
                <input name="phone" className={inputCls} placeholder="+1 (555) 000-0000" />
              </Field>
              <Field label="Job Title">
                <input name="jobTitle" className={inputCls} placeholder="Procurement Manager" />
              </Field>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Company Website">
                <input name="website" className={inputCls} placeholder="acme.com" />
              </Field>
              <Field label="Industry">
                <input name="industry" className={inputCls} placeholder="Software" />
              </Field>
              <Field label="Company Size">
                <div className="relative">
                  <select name="companySize" defaultValue="" className={`${inputCls} appearance-none pr-10`}>
              ...
              <Field label="Tax ID / GST" hint="Optional">
                <input name="taxId" className={inputCls} placeholder="GST123456789" />
              </Field>
            </div>
          </section>
                  ...
                  {id === "billing" ? (
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <Field label="Address Line 1">
                        <input name="addressLine1" className={inputCls} placeholder="123 Market St" />
                      </Field>
                      <Field label="Address Line 2">
                        <input name="addressLine2" className={inputCls} placeholder="Suite 400" />
                      </Field>
                      <Field label="City">
                        <input name="city" className={inputCls} placeholder="San Francisco" />
                      </Field>
                      <Field label="State">
                        <input name="state" className={inputCls} placeholder="CA" />
                      </Field>
                      <Field label="Postal Code">
                        <input name="postalCode" className={inputCls} placeholder="94103" />
                      </Field>
                      <Field label="Country">
                        <input name="country" className={inputCls} placeholder="United States" />
                      </Field>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      ...
                      <Field label="Internal Tags" hint="Comma-separated">
                        <input name="tags" className={inputCls} placeholder="vip, retainer" />
                      </Field>
                      <Field label="Preferred Currency">
                        <div className="relative">
                          <select name="preferredCurrency" defaultValue="" className={`${inputCls} appearance-none pr-10`}>
                      ...
                      <Field label="Payment Terms">
                        <div className="relative">
                          <select name="paymentTerms" defaultValue="" className={`${inputCls} appearance-none pr-10`}>
                            <option value="">Select…</option>
        ...
        <aside className="flex flex-col gap-5">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-xs text-gray-400">
              Saved clients appear here once created.
            </p>
